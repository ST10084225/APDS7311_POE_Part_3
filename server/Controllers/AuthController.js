...
module.exports.Login = async (req, res, next) => {
    try {
      ...
      if(!user){
        return res.status(401).json({error:'Incorrect password or email' }) //user doesn't exist
      }

      const auth = await bcrypt.compare(password,user.password) // if user exists, compare input password to user password
      if (!auth) {
        return res.status(401).json({error:'Incorrect password or email' }) //obscure failed password login attempt --> stop email collection attacks
      }
